...
class Config:
    ...
    def _load_config(self):
        ...
        try:
            ...
            raise Exception(
                status_code=500,
                detail="Houve uma falha crítica do sistema ao ler um arquivo de configuração!",
            )
        except json.JSONDecodeError:
            raise Exception(
                status_code=500,
                detail="Houve uma falha crítica do sistema ao decodificar o arquivo de configuração!",
            )
        except Exception as e:
            raise Exception(
                status_code=500,
                detail="Houve uma falha crítica do sistema ao ler/decodificar o arquivo de configuração!",
            )
    ...
    def get_category(self, config: list[ConfigResponse], category: str) -> list[ConfigResponse]:
        if category is not None:
            config = [c for c in config if c.category == category.value]
            return config
